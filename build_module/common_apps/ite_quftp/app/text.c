...
char *getnextword(char *text, int state)
{
    ...
    {
        ftplog(LOG_CRITICAL, "bad string start... error in program logic!\n");
        exit(0);
    }
...
int parseurl(char *url, struct ftpconnection *connection)
{
    ...
    if ((temp = strstr(url, "://")))
    {
        ...
        {
            ftplog(LOG_CRITICAL, "ftp is the only supported protocol\n");
            return 0;
        }
    }
    else
    ...
    {
        connection->port = ftp_port; //from ipnc set port
    }

    /* The rest is the host name */
    if (strlen(tempurl) == 0)
    {
        ftplog(LOG_CRITICAL, "Hostname is too short!\n");
        free(originalurl);
        return 0;
    }
