...
struct ftpconnection *ftp_new(void)
{
    ...
    ftplog(LOG_CALLS, "Creating new ftp structure at %p", connection);
...
int ftp_connect(struct ftpconnection *connection)
{
    ...
    {
        ftplog(LOG_CRITICAL, "Must initialize struct ftpconnection first\n");
        return -1;
    }
    if (!(hp = (struct hostent *)gethostbyname(connection->hostname)))
    {
        ftplog(LOG_ALERT, "Can't resolve %s: %s\n", connection->hostname, HERROR);
        return -1;
    }
    ...
    ftplog(LOG_MESSAGE, "Resolving %s", connection->hostname);
    fflush(stdout);
    ...
    ftplog(LOG_MESSAGE, "%s resolves to %s", connection->hostname, ipaddr);
    struct timeval timeo = {5, 0};
    socklen_t socklen = sizeof(timeo);
    setsockopt(connection->controlfd, SOL_SOCKET, SO_SNDTIMEO, &timeo, socklen);

    if (connect(connection->controlfd, (SOCKADDR *)&controladdr, sizeof(controladdr)) < 0)
    {
        ftplog(LOG_ALERT, "Connection refused to %s\n", connection->hostname);
        return -1;
    }
    ...
    while (!readynow)
    {
        ...
        {
            case 120 :
                ftplog(LOG_ALERT, "Service not yet ready.. waiting for 5 seconds");
                sleep(5);
                break;
            case 220 :
                readynow = 1;
                ftplog(LOG_MESSAGE, "Connected to %s on port %d",
                       connection->hostname, connection->port);
                break;
            default  :
                ftplog(LOG_MESSAGE, "%s", ftp_error_message(response));
                return response;
                break;
        }
    }
...
int ftp_disconnect(struct ftpconnection *connection)
{
    if (connection->status >= STATUS_IDLE)
    {
        ...
        if (response == 500)
        {
            ftplog(LOG_MESSAGE, "Braindead server doesn't understand QUIT.. closing connection\n");
        }
    ...
    //ftplog(LOG_NOTICE, "Disconnected from %s", connection->hostname);
    connection->controlfd = 0;
    return 0;
}
...
int ftp_reset(struct ftpconnection *connection)
{
    ...
    {
        ftplog(LOG_WARNING, "Still connected. Call ftp_disconnect() first!\n");
        return 1;
    }
...
char *ftp_readline(struct ftpconnection *connection)
{
    ...
    {
        ftplog(LOG_CRITICAL, "ftp_readline: Not connected\n");
        return 0;
    }
    temp = buffer;
    fstep_log("ftp_readline -->while read begin");
    ...
    fstep_log("ftp_readline -->while read over");
    if (n < 0)
    {
        perror("read");
        exit(0);
    ...
    if (n == 0)
    {
        ftplog(LOG_WARNING, "FTP server disconnected");
        return NULL;
    }
...
int ftp_sendline(struct ftpconnection *connection, char *line)
{
    ...
    if (connection->status <= STATUS_WAIT_CONNECT)
    {
        ftplog(LOG_CRITICAL, "ftp_sendline: Not connected\n");
        return 0;
    }
...
int ftp_getrc(struct ftpconnection *connection, char *data, int max_size, int print_line)
{
    ...
    if (connection->status <= STATUS_WAIT_CONNECT)
    {
        ftplog(LOG_CRITICAL, "ftp_getrc: Not connected");
        return 0;
    }
    fstep_log("ftp_getrc -->while ftp_readline begin");
    while ((line = ftp_readline(connection)))
    {
        if (!*line)
        {
            ftplog(LOG_CRITICAL, "Big bad error.. empty string returned by ftp_readline\n");
            break;
        }
        if (print_line)
        {
            if (line[0] == ' ')
            {
                ftplog(LOG_WARNING, "%s", (char *)(line + 1));
            }
            else
            {
                ftplog(LOG_WARNING, "%s", (char *)(line + 4));
            }
        }
    ...
    fstep_log("ftp_getrc -->while ftp_readline over");
    if (!line || !*line) { return 0; }
    sscanf(line, "%d ", &response);
    ftplog(LOG_INFO, "ftp_getrc() = %d", response);
    if (data)
    {
...
int ftp_check_ready(struct ftpconnection *connection, int print)
{
    if (connection->status <= STATUS_WAIT_CONNECT)
    {
        if (print) { ftplog(LOG_CRITICAL, "Not connected\n"); }
        return 0;
    }
    if (connection->status == STATUS_WAIT_LOGIN)
    {
        if (print) { ftplog(LOG_ALERT, "Not logged in\n"); }
        return 0;
    }
    if (connection->status == STATUS_ERROR)
    {
        if (print) { ftplog(LOG_ALERT, "Connection is in error state!\n"); }
        return 0;
    }
    if (connection->status >= STATUS_WAITING)
    {
        if (print) { ftplog(LOG_ALERT, "Server is busy. Please wait\n"); }
        return 0;
    }
    return 1;
}
...
int ftp_stat(struct ftpconnection *connection, char *filename, struct filedetails *details)
{
    ...
    switch (result)
    {
        ...
        case 250 :
            break;
        case 500 :
            ftplog(LOG_ALERT, "Server doesn't understand STAT\n");
            return result;
            break;
        case 501 :
            ftplog(LOG_ALERT, "Bad parameters to STAT\n");
            return result;
            break;
        case 502 :
            ftplog(LOG_ALERT, "Server doesn't understand STAT\n");
            ...
            break;
        case 421 :
            ftplog(LOG_ALERT, "Service not available\n");
            return result;
            break;
        default  :
            ftplog(LOG_ALERT, "Unknown response to STAT: %d\n", result);
            return result;
            break;
    }
    if (buffer[3] == '-')
    {
        ...
        if (!buffer)
        {
            ftplog(LOG_CRITICAL, "Disconnected!\n");
    ...
    {
        ftplog(LOG_ALERT, "Bad response to STAT: %s\n", buffer);
    }
    return 0;
}
    ...
    while (stat_formats[stat_index])
    {
        ...
        while (*format_p)
        {
            ...
            if (*format_p == '%')
            {
                ...
                {
                    case '%' :
                        if (*buffer_p == '%')    /* %% means a literal % character */
                        {
                            buffer_p++;
                        }
                        else
                    ...
                    case 'P' :
                    {
                        char *permissions_p; /* UNIX style permissions */
                    ...
                    case 'N' :
                    {
                        char *filename_p; /* UNIX style filename */
                    ...
                    case 'M' :
                    {
                        char *month_p; /* UNIX style month name */
                    ...
                    case 'n' :
                    {
                        char *month_p; /* UNIX style month number */
                        char *month = (char *)malloc(4096);
                        memset(month, 0, 4096);
                        month_p = month;
                        format_p++;
                    ...
                    case 'm' :
                    {
                        char *minute_p; /* UNIX style minute number */
                    ...
                    case 'D' :
                    {
                        char *mday_p; /* UNIX style mday number */
                    ...
                    case 's' :
                    {
                        char *second_p; /* UNIX style second number */
                    ...
                    case 'h' :
                    {
                        char *hour_p; /* UNIX style hour number */
                    ...
                    default  :
                        ftplog(LOG_CRITICAL, "Unknown character %c in STAT format description\n", *format_p);
                        break;
                }
            }
...
int ftp_delete(struct ftpconnection *connection, char *filename)
{
    ...
    {
        case 250 :
            ftplog(LOG_INFO, "Deleted %s\n", filename);
            return 1;
            break;
        case 450 :
            ftplog(LOG_ALERT, "Can't delete %s: %s\n", filename, resultstring);
            break;
        case 421 :
            ftplog(LOG_ALERT, "%s\n", resultstring);
            break;
        case 500 :
            ftplog(LOG_ALERT, "This server does not support the DELE command\n");
            break;
        case 501 :
            ftplog(LOG_ALERT, "Invalid parameters to DELE command\n");
            break;
        case 502 :
            ftplog(LOG_ALERT, "This server does not support the DELE command\n");
            break;
        case 530 :
            ftplog(LOG_ALERT, "Not logged in correctly\n");
            break;
        case 550 :
            ftplog(LOG_ALERT, "Can't delete %s: %s\n", filename, resultstring);
            break;
        case 553 :
            ftplog(LOG_ALERT, "Not allowed to delete %s\n", filename);
            break;
        default  :
            ftplog(LOG_ALERT, "Unrecognised response to DELE: %d %s\n", result, resultstring);
            break;
    }
    return 0;
}
...
int ftp_exists(struct ftpconnection *connection, char *filename)
{
    ...
    ftplog(LOG_CALLS, "(%p, %s)", connection, filename);
    sprintf(line, "STAT %s\r\n", filename);
    ftp_sendline(connection, line);
...
char *ftp_error_message(int error)
{
    switch (error)
    {
        ...
        case 225 :
            ...
            break;
        case 226 :
            return "Closing data connection successfully.";
            break;
        case 227 :
